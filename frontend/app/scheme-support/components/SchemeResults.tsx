...
export default function SchemeResults({ schemes }: { schemes: Scheme[] }) {
  return (
    <>
      {schemes.length === 0 ? (
        <p className="text-red-600">No schemes matched your details.</p>
      ) : (
        <div className="p-4 space-y-4">
          {schemes.map((scheme) => (
            <div
              key={scheme.id}
              className="border border-pink-200 p-5 rounded-2xl shadow-sm bg-pink-50 hover:shadow-lg transition-shadow duration-300"
            >
      ...
      {/* Footer (outside results) */}
      <footer className="w-full text-center mt-10 py-4 text-sm text-gray-500">
        © {new Date().getFullYear()} Sahaya Sakhi. All rights reserved to{" "}
        <span className="text-pink-700 font-semibold">Anuhya Mattaparthi</span>.
      </footer>
    </>
  );
}
